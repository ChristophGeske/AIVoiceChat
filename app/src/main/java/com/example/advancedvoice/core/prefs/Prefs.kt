...
object Prefs {
    ...
    // API KEYS
    fun getGeminiKey(context: Context): String = sp(context).getString(PrefsKeys.GEMINI_KEY, "") ?: ""
    fun setGeminiKey(context: Context, value: String) = sp(context).edit().putString(PrefsKeys.GEMINI_KEY, value).apply()

    fun getOpenAiKey(context: Context): String = sp(context).getString(PrefsKeys.OPENAI_KEY, "") ?: ""
    fun setOpenAiKey(context: Context, value: String) = sp(context).edit().putString(PrefsKeys.OPENAI_KEY, value).apply()

    // MODELS
    fun getSelectedModel(context: Context): String =
        sp(context).getString(PrefsKeys.SELECTED_MODEL, "gemini-2.5-pro") ?: "gemini-2.5-pro"
